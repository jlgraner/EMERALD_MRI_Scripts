...
def main():
    ...
    base_input_dir = os.path.join(this_env['EMDIR'], 'Data/MRI/DTI_UserTest_09182018_BIDS/dwi_analysis/sub-{sub}/DWI')
    input_file_template = 'sub-{sub}_ses-usertest_run-{run}_{tract}_{sift}meanFA.txt'
    wholebrain_input_file_template = 'sub-{sub}_ses-usertest_run-{run}_wholebrain_{wbsift}meanFA.txt'

    output_dir = os.path.join(this_env['EMDIR'], 'Data/MRI/DTI_UserTest_09182018_BIDS/dwi_analysis')
    subwise_output_file_template = 'run-{run}_{tract}_sub-wise_group_FA.csv'
    ...
    for run in run_list:
        for sub in sub_list:
            ...
            input_file = os.path.join(sub_input_dir, wholebrain_input_file_template.format(sub=sub, run=run, wbsift=wholebrain_sift_prefix))
            ...
            for tract in tract_list:
                input_file = os.path.join(sub_input_dir, input_file_template.format(sub=sub,run=run,tract=tract,sift=sift_prefix))
        ...
        for tract in tract_list:
            ...
            subwise_output_file = os.path.join(output_dir, subwise_output_file_template.format(run=run,tract=tract))
