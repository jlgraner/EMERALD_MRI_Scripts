...
base_input_dir = os.path.join(this_env['EMDIR'], 'Data/MRI/DTI_UserTest_09182018_BIDS/dwiprep/sub-{sub}/ses-{ses}/')
base_output_dir = os.path.join(this_env['EMDIR'], 'Data/MRI/DTI_UserTest_09182018_BIDS/dwi_analysis/sub-{sub}/DWI/')
base_anat_dir = os.path.join(this_env['EMDIR'], 'Data/MRI/DTI_UserTest_09182018_BIDS/fmriprep/fmriprep/sub-{sub}/anat/')

tract_mask_dir = os.path.join(this_env['EMDIR'], 'Analysis/MRI/DTI_TractCreationMasks/')

#Templates for input file names
dti_in_temp = 'sub-{sub}_ses-{ses}_run-{run}_dwi_d_ss_prep_ss_bc_mask.nii.gz'
fod_in_temp = 'sub-{sub}_ses-{ses}_run-{run}_dwi_d_ss_prep_ss_bc_FOD.nii.gz'
bvec_temp = 'sub-{sub}_ses-{ses}_run-{run}_dwi_prep.bvec'
bval_temp = 'sub-{sub}_ses-{ses}_run-{run}_dwi_prep.bval'
...
tckgen_out_temp = 'sub-{sub}_ses-{ses}_run-{run}_wholebrain.tck'
...
for sub in sub_list:
  ...
  for run in run_list:
    ...
    input_dti = os.path.join(sub_input_dir, dti_in_temp.format(sub=sub,ses=ses,run=run))
    input_fod = os.path.join(sub_input_dir, fod_in_temp.format(sub=sub,ses=ses,run=run))
    bvec = os.path.join(sub_input_dir, bvec_temp.format(sub=sub,ses=ses,run=run))
    bval = os.path.join(sub_input_dir, bval_temp.format(sub=sub,ses=ses,run=run))
    # transform_file = os.path.join(sub_anat_dir, transform_temp.format(sub=sub, ses=ses))
    ref_image = os.path.join(sub_input_dir, fa_temp.format(sub=sub,ses=ses,run=run))
    ...
    try:
     ...
     tckgen_out = os.path.join(sub_output_dir, tckgen_out_temp.format(sub=sub, ses=ses, run=run))
