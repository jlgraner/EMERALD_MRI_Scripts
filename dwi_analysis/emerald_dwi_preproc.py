...
def preproc(input_file, output_dir, bvec_file, bval_file, b0_image_file):
    ...
    if b0_image_file is None:
        call_parts = ['dwipreproc', input_file, output_file, '-tempdir', output_dir, '-rpe_none', '-pe_dir', 'AP', '-fslgrad', bvec_file, bval_file, '-export_grad_fsl', output_bvec, output_bval, '-force']
    else:
        call_parts = ['dwipreproc', input_file, output_file, '-tempdir', output_dir, '-rpe_pair', '-se_epi', b0_image_file, '-pe_dir', 'AP', '-fslgrad', bvec_file, bval_file, '-export_grad_fsl', output_bvec, output_bval, '-force']
    logging.info('Running: {}'.format(string.join(call_parts)))
