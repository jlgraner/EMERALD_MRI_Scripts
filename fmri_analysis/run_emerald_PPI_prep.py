...
for sub in sub_list:
    ...
    for run in run_list:
        ...
        temp_mean_image = os.path.join(sub_input_dir, 'sub-{sub}_emoreg_run{run}_AROMApreproc_mean.nii.gz'.format(sub=sub, roi=roi))
        ...
        x_array, y_array = emlab.create_histo(temp_mean_image)
        ...
        for roi in roi_list:
            ...
            output_file = os.path.join(sub_input_dir, 'sub_{}_run{}_ROI_{}_final.nii.gz'.format(sub,run,roi))
    ...
    for roi in roi_list:
        mask_list = []
        roi_file = os.path.join(roi_dir, 'ROI_{}_final.nii.gz'.format(roi))
        final_output_roi = os.path.join(sub_input_dir, 'sub_{}_ROI_{}_final.nii.gz'.format(sub,roi))
        mul_part = ''
