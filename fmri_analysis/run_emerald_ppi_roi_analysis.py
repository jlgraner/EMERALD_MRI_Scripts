...
input_dir = os.path.join(this_env['EMDIR'], 'Analysis/MRI/sub-{sub}/Func/Second_level_PPI_allruns.gfeat')
feat_output_dir = 'featquery_{roi}_output'

base_output_dir = os.path.join(this_env['EMDIR'], 'Analysis/MRI/ROI_PPI_Analysis_Output/Participant_Output')
output_file_template = '{sub}_ROI_PPImeans_fsl_{cope}.csv'

roi_dir = os.path.join(this_env['EMDIR'], 'Analysis', 'MRI', 'ROIs')

#for each subject:
#	1) Locate the Dist>flow amy-PPI contrast map
#	2) Locate the Reap>flow amy-PPI contrast map
#	3) For the Dist>flow amy-PPI contrast map, find the average value in the dlPFC.
# 4) For the Reap>flow amy-PPI contrast map, find the average value in the vlPFC.
#	5) Extract average map values
#	6) Save the average values into an output file
...
for sub in subs_to_run:
    ...
    if not os.path.exists(input_dir.format(sub=sub)):
        print('Input directory cannot be found for subject {}!'.format(sub,input_dir.format(sub=sub)))
        raise RuntimeError

    #Look for the output for cope1 (reap>flow) and cope2 (dist>flow)
    for key in cope_labels:
        cope_file = os.path.join(input_dir.format(sub=sub), 'cope{}.feat'.format(cope_labels[key]), 'stats', 'cope1.nii.gz')
    ...
    for roi in roi_list:
      ...
      roi_file = os.path.join(roi_dir, 'ROI_{roi}_final.nii.gz'.format(roi=roi))
      if not os.path.exists(roi_file):
        print('Input ROI mask image cannot be found! File: {file}'.format(file=roi_file))
        raise RuntimeError
...
for sub in subs_to_run:
    ...
    for cope in cope_labels:
        ...
        cope_dir = os.path.join(input_dir.format(sub=sub), 'cope{}.feat'.format(cope_labels[cope]))
