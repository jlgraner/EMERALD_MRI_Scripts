...
template_directory = os.path.join(this_env['EMDIR'], 'Scripts/MRI_Analysis/fsl_templates/first_level_motorevent_UT_template_files_09242019/')
template_file = 'first_level_design_template_UT_motorevent.fsf'
...
for sub in subs_to_run:
    for run in runs_to_run:
        run_template_file = os.path.join(template_directory, '{sub}_run{run}_motorevent_first_level_design.fsf'.format(sub=sub,run=run))
        ...
        try:
            ...
            this_feat_dir = os.path.join(this_env['EMDIR'], 'Analysis/MRI/sub-{sub}/Func/First_level_motorevent_run{run}.feat'.format(sub=sub,run=run))
